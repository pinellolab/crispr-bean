...
def get_outlier_guides_and_mask(
    ...
    condit_col: column in bdata.condit with experimental conditions
    replicate_col: column in bdata.condit with experimental replicate
    ...
    outlier_guides[replicate_col] = bdata.condit.loc[
        outlier_guides["sample"], replicate_col
    ].values
    mask = pd.DataFrame(
        index=bdata.guides.index, columns=bdata.condit[replicate_col].unique()
    ).fillna(1)
